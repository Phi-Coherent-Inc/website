...
export const PhiQudit = {
  ...
  // GATE synthesis at a single dimension d: compile SU(d) targets into the native
  // gate set {X_d,F_d,T_d}. Mean error per search strategy (one-directional beam vs
  // co-adaptive) at a matched budget. Returns [{ strategy, dim, error, gateLength,
  // nTargets }] (2 rows). ~sub-second at d≤12 — run deferred.
  gateSynthScoreboard(d = 10) { return vecToArray(_module.runQuditGateSynthScoreboard(d >>> 0)); },
};
